...
int main(int argc, char** argv){
	...
	i = fork();
	if(!i){
		system("client/WTF configure 127.0.0.1 9000");
		exit(0);
	}
	wait();
	...
	if(!i){
		...
		system("client/WTF add test a.txt");
		system("client/WTF add test b.txt");
		system("client/WTF add test c.txt");
		exit(0);
	}
	wait();
	...
	i = fork();
	if(!i){
		system("client/WTF upgrade test");
		exit(0);
	}
	wait();
	...
	i = fork();
	if(!i){
		system("client/WTF remove test a.txt");
		exit(0);
	}
	wait();
	...
	i = fork();
	if(!i){
		system("client/WTF update test a.txt");
		exit(0);
	}
	wait();
	...
	i = fork();
	if(!i){
		system("client/WTF destroy test");
		exit(0);
	}
	wait();
